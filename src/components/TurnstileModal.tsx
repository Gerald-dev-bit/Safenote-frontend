...
const TurnstileModal: React.FC<TurnstileModalProps> = ({ onVerify }) => {
  ...
  useEffect(() => {
    ...
    // Poll for window.turnstile (script preloaded in <head>)
    ...
    // Fail-safe timeout (5s max wait; optional 3s min if you want delay—comment out for instant)
    const timeout = setTimeout(() => {
      clearInterval(pollInterval);
      setIsLoading(false);
      init(); // Force render
    }, 5000); // Change to 3000 for enforced 3s min delay if needed
